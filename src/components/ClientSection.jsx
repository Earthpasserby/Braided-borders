        ...
        <div className="grid gap-14 md:grid-cols-[0.65fr_1.35fr] md:gap-28">
          ...
          <div className="grid gap-0 border-t border-brand-ink/20">
            {clientTypes.map((client, index) => (
              <div
                key={client}
                className="flex gap-6 border-b border-brand-ink/20 py-7"
              >
          ...
          <div>
            ...
            <div className="mt-7 grid grid-cols-2 gap-3">
              {sectors.map((sector) => (
                <span
                  key={sector}
                  className="border border-brand-ink/20 px-4 py-4 text-display text-2xl"
                >
                  {sector}
                </span>
              ))}
          ...
          <div>
            ...
            <ul className="mt-7 border-t border-brand-ink/20">
              {networkRelationships.map((relationship) => (
                <li
                  key={relationship}
                  className="border-b border-brand-ink/20 py-4 text-base"
                >
                  {relationship}
                </li>
              ))}
