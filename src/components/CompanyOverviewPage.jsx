    ...
    <main className="bg-brand-white text-brand-ink">
      <header className="border-b border-brand-ink/10 bg-white/90 backdrop-blur-md">
        <div className="mx-auto flex max-w-[1440px] items-center justify-between px-6 py-5 md:px-12">
          <a href="#top" className="flex items-center gap-3">
            ...
            />
            <span className="text-mono text-[10px] uppercase tracking-[0.16em] text-brand-ink">
              Braided Borders
            </span>
          </a>
      ...
      <section className="px-6 py-16 md:px-12 md:py-24">
        <div className="mx-auto max-w-[1200px]">
          <p className="text-mono text-[10px] uppercase tracking-[0.22em] text-brand-blue">
            About the firm
          </p>
          ...
          <p className="mt-8 max-w-3xl text-lg leading-8 text-brand-ink/70">
            ...
            of deploying capital across Africa and the Middle East, combining
            on-the-ground intelligence, operational support, and ecosystem
            access into a single strategic platform.
          </p>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-[1200px]">
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {capabilities.map((capability, index) => (
              ...
              >
                <span className="text-mono text-[10px] text-brand-blue">
                  0{index + 1}
                </span>
            ...
            <div className="mt-7 grid gap-4 md:grid-cols-3">
              {startupServices.map((service, index) => (
                <div
                  ...
                  className="rounded-2xl border border-white/15 bg-white/5 p-5"
                >
                  <span className="text-mono text-xs text-white/60">
                    0{index + 1}
                  </span>
        ...
        <div className="mx-auto max-w-[1200px]">
          ...
          <div className="grid gap-0 rounded-3xl border border-brand-ink/10 overflow-hidden">
            {clientTypes.map((client, index) => (
              <div
                key={client}
                className="flex gap-6 border-b border-brand-ink/10 bg-white px-5 py-6 last:border-b-0 md:px-8"
              >
        ...
        <div className="mx-auto max-w-[1200px]">
          <div className="mb-14 flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
            ...
            <p className="max-w-sm text-base leading-7 text-white/75">
              Three distinct layers of execution, from investor intelligence to
              local institutional growth.
            </p>
          </div>
          ...
          <div className="grid gap-5 lg:grid-cols-3">
            {phases.map((phase) => (
              <article
                key={phase.number}
                className="rounded-2xl border border-white/30 bg-white/5 p-6"
              >
                <span className="text-mono text-xs text-white/75">
                  PHASE {phase.number}
                </span>
            ...
            <div>
              ...
              <div className="mt-7 grid grid-cols-2 gap-3">
                {sectors.map((sector) => (
                  <span
                    key={sector}
                    className="rounded-2xl border border-brand-ink/15 bg-brand-white px-4 py-4 text-display text-2xl"
                  >
                    {sector}
                  </span>
                ))}
            ...
            <div>
              ...
              <ul className="mt-7 space-y-3">
                {networkRelationships.map((relationship) => (
                  <li
                    key={relationship}
                    className="rounded-2xl border border-brand-ink/10 bg-[#f6f8f8] px-4 py-4 text-base"
                  >
                    {relationship}
                  </li>
                ))}
        ...
        <div className="mx-auto max-w-[1200px]">
          ...
          <div className="grid gap-8 md:grid-cols-2">
            {founders.map((founder) => (
              <article
                key={founder.name}
                className="rounded-[28px] border border-brand-ink/10 bg-white p-5 shadow-sm"
              >
                ...
                <p className="text-mono text-[10px] uppercase tracking-[0.16em] text-brand-blue">
                  {founder.region}
                </p>
